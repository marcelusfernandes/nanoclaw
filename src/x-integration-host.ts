...
export async function handleXIpc(
  ...
  switch (type) {
    ...
    case 'x_search':
      ...
      result = await runScript('search', { query: data.query, count: data.count || 10, tab: data.tab || 'top' });
      break;
    ...
    case 'x_read_tweet':
      ...
      result = await runScript('read-tweet', { tweetUrl: data.tweetUrl });
      break;
    ...
    case 'x_read_replies':
      ...
      result = await runScript('read-replies', { tweetUrl: data.tweetUrl, count: data.count || 10 });
      break;

    case 'x_trending':
      result = await runScript('trending', { count: data.count || 20 });
      break;
    ...
    case 'x_user_posts':
      ...
      result = await runScript('user-posts', { username: data.username, count: data.count || 10, tab: data.tab || 'posts' });
      break;
